...
这是从 GitHub 或压缩包获取的独立插件目录。Codex 推荐使用仓库根目录的 marketplace 安装命令；支持插件目录的其他平台可以直接读取本目录。压缩包同时包含 `.codex-plugin` 和兼容 Claude Code 的 `.claude-plugin` manifest。
...
插件不会把“用了 LangGraph/向量库/Tool”直接当成亮点。它先从源码或口述事实生成链路能力卡，再解释为什么这条链路解决了生产级问题：模型输出如何被约束、任务中断如何恢复、外部副作用如何避免重复、知识变化如何回滚、质量变化如何回归。
...
业务问题与失败后果
→ 输入/成功终态/失败终态
→ 编排、状态、Tool/RAG、评测/观测机制
→ 为什么比普通 API 或 Prompt 更难
→ 真实源码证据和个人负责范围
→ 适合的业务对象/生命周期
→ 目标项目源码嵌入点
→ 最小重实现、验证方式和指标
...
| 链路 | 亮点是什么 | 适合融合的业务背景 |
|---|---|---|
| 长程任务与持久化编排 | 任务跨进程/跨天运行，靠 checkpoint、恢复、分类重试、幂等和人工接管形成可运营闭环 | 合规整改、合同履约、发布风险门禁、跨系统异常处置 |
| Tool 调用可靠性与安全边界 | 把不可信模型输出约束为有 schema、权限、超时、审计和副作用补偿的系统动作 | 指标口径治理、供应商准入、合规案件、研发发布 |
| 评测、轨迹与回归体系 | 用数据集、trajectory、grader、阈值和失败回流证明版本改动，而不是靠 Demo 主观判断 | 知识变更发布、问数审计、研究证据链、异常处置 |

记忆生命周期、RAG 版本/权限、可观测性与成本治理、多 Agent 人机协同和安全策略属于 A 级，必须有跨层证据才写成亮点。普通向量检索、单次调用、Prompt 模板和没有失败/状态/评测的多角色展示只是配套能力。

### 业务承载不是“改名”

选择 GitHub 项目时，插件会检查业务对象、生命周期、系统副作用和源码嵌入点：

```text
曼弗实习真实事实 → 高价值链路能力卡 → 候选项目业务对象/生命周期
→ 候选源码嵌入点 → 用户实际重实现 → 测试/日志/评测 → 独立项目包装
```

例如，实习中真实接触的是 Tool 超时和重复执行问题，可以在一个“研发变更风险门禁”项目中重实现审批 checkpoint、发布幂等和回滚测试；不能把 GitHub 项目已有代码写成曼弗生产能力。每个候选都要同时给出“选择理由”和“淘汰理由”，并分开列出“已有能力、建议改造、完成后可写、当前不可写”。

详细定义、各链路源码证据、业务背景卡片和评分矩阵见 [高价值 Agent 链路](references/high-value-agent-chains.md) 与 [经历承载项目参考](references/business-carrier-scenarios.md)。
...
短名单必须按下面的格式比较，而不是只列项目名称：

```text
候选项目 → 业务对象/生命周期 → 能承载的实习链路
→ 为什么自然 → 源码嵌入点 → 最小重实现 → 可验证指标
→ 选择理由 / 淘汰理由
```

业务承载自然度占 30%，链路嵌入点占 25%，源码证据密度占 20%，业务独特性占 15%，可实现范围占 10%；star 只是弱信号。只有用户确认候选后才拉取源码，最终推荐必须来自 cloned manifest，并有至少 5 个跨 3 段的本地证据点。
